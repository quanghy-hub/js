...
(function () {
    ...
    const css = `
        ...
        #fvp-wrapper video { 
            ...
            pointer-events: none;
            background: #000; /* Ensure black background for video area */
    ...
    const getCoord = e => {
        ...
        return { x: t.clientX, y: t.clientY, sx: t.screenX, sy: t.screenY };
    };
    ...
    const state = {
        ...
        isFrameFloat: false, frame: null, framePh: null, frameStyles: {}
    };
    ...
    const restore = () => {
        ...
        if (state.isFrameFloat && state.frame) {
            state.frame.style.cssText = state.frameStyles.cssText;
            state.framePh?.remove();
            box.style.cssText = ''; // Clear inline styles
            box.style.display = 'none'; // Ensure hidden
            document.body.style.overflow = ''; // Restore scroll
            state.isFrameFloat = false; state.frame = null; state.framePh = null; state.frameStyles = {};
        }
    ...
    const float = v => {
        ...
        // Iframe Floating Logic
        const frame = window.frameElement;
        if (frame && window.self !== window.top) {
            try {
                state.isFrameFloat = true;
                state.frame = frame;
                state.frameStyles = { cssText: frame.style.cssText, width: frame.offsetWidth, height: frame.offsetHeight };

                const r = frame.getBoundingClientRect();
                state.framePh = document.createElement('div');
                state.framePh.style.cssText = `width:${r.width}px;height:${r.height}px;display:${getComputedStyle(frame).display}`;
                frame.parentNode.insertBefore(state.framePh, frame);

                Object.assign(frame.style, {
                    position: 'fixed', zIndex: 2147483647,
                    width: '320px', height: '180px',
                    top: `${clamp((window.parent.innerHeight - 180) / 2, 20, window.parent.innerHeight - 200)}px`,
                    left: '20px', right: 'auto', bottom: 'auto', margin: '0',
                    maxWidth: '100vw', maxHeight: '100vh',
                    transition: 'width 0.2s, height 0.2s' // Smooth resize
                });
            } catch (e) { console.log('Iframe Float failed', e); state.isFrameFloat = false; }
        }
        ...
        if (state.isFrameFloat) {
            // Ensure box fills the iframe
            ...
            // Force redraw/layout of iframe contents if needed
            document.body.style.overflow = 'hidden'; // Hide original scrollbars inside iframe
        } else {
    ...
    const setupEvents = () => {
        ...
        const move = e => {
            ...
            if (state.isIconDrag) {
                ...
                const dx = c.sx - state.startX, dy = c.sy - state.startY;
                const target = state.isFrameFloat ? state.frame : box;
                const winW = state.isFrameFloat ? window.parent.innerWidth : innerWidth;
                const winH = state.isFrameFloat ? window.parent.innerHeight : innerHeight;

                target.style.left = `${clamp(state.initX + dx, -target.offsetWidth + 60, winW - 60)}px`;
                target.style.top = `${clamp(state.initY + dy, -target.offsetHeight + 60, winH - 60)}px`;
            } else if (state.isResize) {
                const dx = c.sx - state.startX, dy = c.sy - state.startY;
                const target = state.isFrameFloat ? state.frame : box;
                if (state.resizeDir.includes('r')) target.style.width = `${Math.max(200, state.initW + dx)}px`;
                if (state.resizeDir.includes('b')) target.style.height = `${Math.max(120, state.initH + dy)}px`;
            }
        };
        ...
        const startDrag = e => {
            ...
            const c = getCoord(e);
            state.isDrag = true;
            state.startX = c.sx; state.startY = c.sy;
            const target = state.isFrameFloat ? state.frame : box;
            state.initX = target.offsetLeft; state.initY = target.offsetTop;
        };
        ...
        box.querySelectorAll('.fvp-resize-handle').forEach(h => {
            const startResize = e => {
                ...
                state.startX = c.sx; state.startY = c.sy;
                const target = state.isFrameFloat ? state.frame : box;
                state.initW = target.offsetWidth; state.initH = target.offsetHeight;
            };
